...
interface RegisterFormProps {
  onRegister: (email: string, password: string, adminCode?: string) => Promise<boolean>;
  onSwitchToLogin: () => void;
  theme: 'dark' | 'light';
}

export default function RegisterForm({ onRegister, onSwitchToLogin, theme }: RegisterFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await onRegister(email, password, adminCode || undefined);
      if (!success) {
        setError('Error al registrar usuario. Verifique que la contraseña sea única.');
      }
    } catch (err) {
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className={`w-full px-4 py-3 pr-12 rounded-lg border ${inputClasses} focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200`}
              placeholder="Código de acceso de controlador"
            />
            <button
              type="button"
